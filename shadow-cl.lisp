...
(defun redefine-make-package (sandbox old-symbol new-symbol)
  (declare (ignore old-symbol))
  (setf (symbol-function new-symbol)
        (lambda (package-name &key nicknames use) 
          (user-make-package sandbox package-name
                             :nicknames nicknames :use use)))
  new-symbol)

(defparameter *shadow-11-packages*
  '((cl:*package* :ignore)
    (cl:defpackage :ignore)
    (cl:delete-package :ignore)
    (cl:do-all-symbols :ignore)
    (cl:do-external-symbols :ignore)
    (cl:do-symbols :ignore)
    (cl:export :ignore)
    (cl:find-all-symbols :ignore)
    (cl:find-package :ignore)
    (cl:find-symbol :ignore)
    (cl:import :ignore)
    (cl:in-package :ignore)
    (cl:intern :ignore)
    (cl:list-all-packages :ignore)
    (cl:make-package :redefine redefine-make-package)
    (cl:package :ignore)
    (cl:package-error :ignore)
    (cl:package-error-package :ignore)
    (cl:package-name :ignore)
    (cl:package-nicknames :ignore)
    (cl:package-shadowing-symbols :ignore)
    (cl:package-use-list :ignore)
    (cl:package-used-by-list :ignore)
    (cl:packagep :shadow)
    (cl:rename-package :ignore)
    (cl:shadow :ignore)
    (cl:shadowing-import :ignore)
    (cl:unexport :ignore)
    (cl:unintern :ignore)
    (cl:unuse-package :shadow)
    (cl:use-package :shadow)
    (cl:with-package-iterator :ignore)))
